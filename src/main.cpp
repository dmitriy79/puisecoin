...
#if defined(NDEBUG)
# error "Puisecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xf4a727b71d23a817cbb0352e51aa9bfaed6c78209eadff045f8090175866fb63");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Puisecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Puisecoin Signed Message:\n";
...
{
    // Puisecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Puisecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Puisecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1496599810, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Hydh";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("048fbce75da4e505f49d71183a4be69443a43411a10ffd4134b4b2989505af5f7326a53a0882be5c309b17702e75ff182b1e73486a397108d142ddf2a1fcda1355") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1496599810;
        ...
        assert(block.hashMerkleRoot == uint256("0x25f4f92435006753f76dee107dbb991528c6d7ad460e00dea6898b3348e4919d"));
...
unsigned char pchMessageStart[4] = { 0xbd, 0xf3, 0x21, 0x90 }; // Puisecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PuisecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PuisecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PuisecoinMiner(CWallet *pwallet)
{
    printf("PuisecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("puisecoin-miner");
    ...
    try { loop {
        ...
        printf("Running PuisecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PuisecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PuisecoinMiner, pwallet));
}
